...
A personal Chrome extension for daily LeetCode repetitions and confirmed Notion saves. Notion connects directly from the extension: no running bridge or hosted service.
...
The **Log** tab saves an attempt only after you choose an outcome. Review your saved work directly in Notion. Connection and recovery controls live in **Settings**.
...
Bridge-only environment settings are documented under [legacy tools](docs/LEGACY_BRIDGE.md).
...
For existing v1–v3 trackers, follow the [migration and maintenance guide](docs/NOTION_MAINTENANCE.md)
before importing the manifest. The same guide covers latest-Attempt retention, historical cleanup,
and Grind links, including backup and recovery requirements. Existing v4 trackers can proceed below.
...
Daily Reps and idle panels make no Notion requests. The sidebar has only Daily Reps and Log;
use Notion directly for your review queue. Existing imported preferences remain preserved locally.
...
See the [documentation index](docs/README.md) for current operating guides, optional legacy tools,
