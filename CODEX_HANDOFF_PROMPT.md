...
Daily runtime startup is also deliberate: `lc-log.command` runs the existing bridge in a visible
foreground terminal session. Terminal.app is the documented Finder default, not an enforced runtime
dependency; direct shell and alternate-terminal launches remain supported. Do not replace it with a
LaunchAgent, login item, hidden daemon, cloud bridge, native messaging host, or direct Notion token
access unless the user changes scope.
