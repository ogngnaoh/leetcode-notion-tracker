...
export class LatestAttemptStore {
  ...
  private currentRecord(page: any): StoredAttempt {
    const record = this.parse(page);
    const receipt = this.states
      .get(page.id)
      ?.receipts.find((r) => r.value.clientEventId === this.currentId(page))?.value;
    if (!receipt) return record; // Legacy pages may only have a Notion date.
    if (!matchesNotionTimestamp(record.attemptedAt, receipt.attemptedAt))
      throw new Error('Current Attempt timestamp does not match its receipt.');
    return { ...record, attemptedAt: receipt.attemptedAt };
  }
  ...
  async latest(problemKey: string): Promise<StoredAttempt | null> {
    ...
    const record = this.currentRecord(page);
    const firstAttempt = [
      record.attemptedAt,
  ...
  async save(
    ...
    const state = await this.state(page);
    const current = this.currentRecord(page);
  ...
  private async finish(page: any, blocks: any[], receiptId: string, receipt: Receipt) {
    ...
    if (Date.parse(event.attempt.attemptedAt) >= Date.parse(current.attemptedAt)) {
      ...
      if (
        ...
        !matchesNotionTimestamp(updated.attemptedAt, event.attempt.attemptedAt) ||
